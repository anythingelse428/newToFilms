<template>
  <div class="auth">
    <h2>Авторизация</h2>
    <div class="auth__login container-sm">
      
        <input type="email" v-model="mail" placeholder="E-mail" :class="valid.email" />

     
        <input type="password" v-model="password" placeholder="Пароль" :class="valid.password"/>

      <button @click="login(), loginValidate()">Войти</button>
    </div>
    <sub
      >Ещё нет аккаунта?
...
export default {
  ...
  methods: {
    login() {
      Api.login(this, this.mail, this.password)
        .then((data) => {
          if (data?.jwt) {
